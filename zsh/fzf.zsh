...
export KEYTIMEOUT=100  # キーシーケンスの猶予時間（10ms単位 = 1秒）
export FZF_TMUX=1
...
function fzf-history() {
    ...
    local selected_line="$(history -n 1 | eval $tac | $(__fzfcmd) \
    --query "$LBUFFER" \
    --reverse \
    --wrap \
    --track \
...
# wrapを有効にして、fzfのctrl-rを利用する
export FZF_CTRL_R_OPTS=$'--bind ^-/:toggle-wrap --wrap-sign "\t↳ "'

# zを利用し、ディレクトリのあいまい検索を行う
function fzf-z() {
    local current_buffer=$BUFFER
    local selected_line="$(z -l $current_buffer 2>&1 | $(__fzfcmd) +s --tac | awk '{ print $2 }' | head -n 1)"
...
function fzf-files() {
    local current_buffer=$BUFFER
    local result="$(ag -l --hidden --ignore .git -g "" | $(__fzfcmd) \
        --preview 'bat --color always {}' \
        --border --border-label ' enter:paste | C-y:clip | C-a:abs+paste | C-o:abs+clip ' --border-label-pos bottom \
        --expect ctrl-y,ctrl-a,ctrl-o
    )"

    local key=$(head -1 <<< "$result")
    local selected_lines=$(tail -n +2 <<< "$result")

    if [ -n "$selected_lines" ]; then
        if [[ $current_buffer =~ cd ]]; then
            selected_lines=$(dirname "$selected_lines")
        fi
        if [[ "$key" == "ctrl-a" || "$key" == "ctrl-o" ]]; then
            selected_lines=$(echo "$selected_lines" | while read -r f; do realpath "$f"; done)
        fi
        local output=$(echo "$selected_lines" | tr '\n' ' ')
        if [[ "$key" == "ctrl-y" || "$key" == "ctrl-o" ]]; then
            printf '%s' "$output" | pbcopy
        else
            BUFFER="${current_buffer}${output}"
            CURSOR=$#BUFFER
        fi
    fi
}
